...
from ..schemas import ConvertRequest  # noqa: TC001
...
@router.post('/convert')
async def convert_subscription(payload: ConvertRequest, request: Request, db: DbSession) -> dict[str, object]:
    ...
    return await SubscriptionMerger([subscription]).merge(template)
